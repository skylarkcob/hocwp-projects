...
class HOCWP_Table_String_Translation extends WP_List_Table {
	...
	function get_columns() {
		$columns = array(
			...
			'string'       => __( 'String', 'hocwp-theme' ),
			'name'         => __( 'Name', 'hocwp-theme' ),
			'context'      => __( 'Group', 'hocwp-theme' ),
			'translations' => __( 'Translation', 'hocwp-theme' ),
		);
	...
	function get_bulk_actions() {
		return array( 'delete' => __( 'Delete', 'hocwp-theme' ) );
	}
	...
	function extra_tablenav( $which ) {
		...
		printf( '<label class="screen-reader-text" for="select-group" >%s</label>', __( 'Filter by group', 'hocwp-theme' ) );
		echo '<select id="select-group" name="group">' . "\n";
		printf(
			'<option value="-1"%s>%s</option>' . "\n",
			- 1 === $this->group_selected ? ' selected="selected"' : '',
			__( 'View all groups', 'hocwp-theme' )
		);
	...
	function column_cb( $item ) {
		return sprintf(
			...
			esc_attr( hocwp_get_value_by_key( $item, 'row' ) ),
	...
	function column_translations( $item ) {
		$out    = '';
		$string = hocwp_get_value_by_key( $item, 'string' );
		if ( ! empty( $string ) ) {
