...
class HOCWP_Table_String_Translation extends WP_List_Table {
	...
	function get_columns() {
		$columns = array(
			...
			'string' => __('String', 'hocwp'),
			'name' => __('Name', 'hocwp'),
			'context' => __('Group', 'hocwp'),
			'translations' => __('Translation', 'hocwp'),
		);
		return $columns;
	}
	...
	function get_bulk_actions() {
		return array('delete' => __('Delete', 'hocwp'));
	}
	...
	function extra_tablenav($which) {
		...
		printf('<label class="screen-reader-text" for="select-group" >%s</label>', __( 'Filter by group', 'hocwp'));
		echo '<select id="select-group" name="group">' . "\n";
		printf(
			'<option value="-1"%s>%s</option>' . "\n",
			-1 === $this->group_selected ? ' selected="selected"' : '',
			__('View all groups', 'hocwp')
		);
