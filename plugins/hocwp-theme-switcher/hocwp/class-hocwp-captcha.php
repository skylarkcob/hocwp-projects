...
class HOCWP_Captcha {
    ...
    public function generate_image() {
        ...
        $code = hocwp_random_string($this->get_length(), $this->get_chars());
        $filename = '';
        ...
        if($im = @imagecreatetruecolor($size[0], $size[1])) {
            ...
            $filename = md5(hocwp_random_string());
        ...
        if(!empty($filename)) {
            ...
            $data = array(
                ...
                'timestamp' => strtotime(hocwp_get_current_date('Y-m-d H:i:s'))
            );
    ...
    public function check($code) {
        $current_datetime = hocwp_get_current_date('Y-m-d H:i:s');
        $captcha_session = $this->get_session_data();
        $timestamp = hocwp_get_value_by_key($captcha_session, 'timestamp');
        ...
        $save_code = hocwp_get_value_by_key($captcha_session, 'hashed');
